...
export default function handleManyToManyRelations(entity: any, Types: object, EntityClass: new () => any, subEntityPath: string) {
  ...
  Object.entries(entityMetadata).forEach(([fieldName, fieldTypeName]) => {
    ...
    if (
      ...
      entity[fieldPathName] = (entity[fieldPathName] ?? []).map((subEntity: any) => subEntity._id);
